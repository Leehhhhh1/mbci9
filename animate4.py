...
class Animation_EEG(FigureCanvas):
    def __init__(self, width, height, dpi, num_channels):
        ...
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_animate)
    ...
    def update_animate(self):
        ...
        for i in range(self.num_channels):
            ...
            # self.axs[i].set_ylim(-10000,10000)
        self.draw_idle()
        # self.flush_events()
    ...
    #     self.thread_update = Thread(target=self.start_update, args=())
    ...
    def update_y(self, data):
        ...
        end = (self.write_prt + len_data) % self.len
        if end > self.write_prt:
            self.data_eeg[:,self.write_prt:end] = data
        else:
            first = self.len - self.write_prt
            self.data_eeg[:,self.write_prt:] = data[:,:first]
            self.data_eeg[:,:end] = data[:,first:]
        self.write_prt = end
        self.y = np.roll(self.data_eeg,-self.write_prt,axis=1)
        # self.data_eeg = np.hstack((self.data_eeg, data))          #水平连接矩阵
...
class Animation_Spectrogram(FigureCanvas):
    def __init__(self, width=5, height=4, dpi=100, num_channels=8, samples=250):
        self.num_channels = num_channels # 通道数
        self.fig, self.axs = plt.subplots(1, 1, figsize=(4, 15))
